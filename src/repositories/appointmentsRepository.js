...
export const findAppointmentsByDate = async (date_time) =>{
...
export const getBarberAppointments = async (id) => {
  const [rows] = await pool.query('SELECT * FROM appointments WHERE barber_id = ?', [id]);
  return rows.length > 0 ? rows : null;
}

export const putUserAppointments = async (id, {barber_id, service_id, update_time, date_time} ) => {
  let setClause = '';
  let params = [];
...
export const putBarberAppointments = async (id, {client_id, service_id, update_time, date_time} ) => {
  let setClause = '';
  let params = [];
...
export const deleteAppointments = async (id, {date_time}) => {
